...
> 本页是 Niagara 学习路径 [[Wiki/Syntheses/UE/Niagara/Niagara-learning-path]] Phase 3 的**主题读本**——详细、精确、满满当当,一次读完即完整掌握 Niagara 的运行时实例层(状态机 + 三阶段 Tick + 批量调度),不需要跳转。
>
...
| 文件 | 行数 | 层级 | 角色 |
|---|---|---|---|
| `NiagaraEmitterInstance.h` | 238 | 最内层 | 单个 Emitter 的运行时(粒子数据 + Spawn/Update 脚本上下文) |
| `NiagaraSystemInstance.h` | 573 | 中间层 | 单个 System 特效的运行时(状态机 + Emitter 容器 + 三阶段 Tick) |
| `NiagaraSystemSimulation.h` | 428 | 最外层 | 同 Asset 多实例的**批量调度器**(Tick Batch + 参数聚合 DataSet) |
...
先回答问题 1。`FNiagaraSystemInstance`、`FNiagaraEmitterInstance`、`FNiagaraSystemSimulation` 三个类都是**普通 C++ 类**,不是 [[Wiki/Concepts/UE/UE4-uobject-系统|UObject]]。
...
对外的"结束"API 有三个(L66-68),作用有别:

```cpp
void Deactivate(bool bImmediate = false);    // bImmediate=false 软结束(Requested→Inactive);bImmediate=true 硬结束
void Complete(bool bExternalCompletion);     // 显式标记完成(bExternalCompletion 影响是否广播 OnCompleteDelegate)
void OnPooledReuse(UWorld& NewWorld);        // Pool 场景下的"从另一个 World 捡回来"复用入口
```

Pool 场景用 `bPooled : 1`(L480)标记。注释原文:`"True if the system instance is pooled. Prevents unbinding of parameters on completing the system"`——**池化实例 Complete 时不 Unbind 参数**,留给下次 Reuse 直接用。细节在 Phase 9。
...
- **`ManualTick(DeltaSeconds, GraphEvent)`(L117)vs `AdvanceSimulation(TickCountToSimulate, TickDeltaSeconds)`(L253)——这是两个不同方法**。`ManualTick` 是**单步**推进(供 Solo 模式的一次 tick);`AdvanceSimulation` 是**循环调 N 次**的高层 API,Component 的 `AdvanceSimulation` BP 节点底下走这条
- `SetSolo(bool)` —— 切换 Solo;触发 `TransferInstance` 在 Simulation 之间迁移(§5)
- `RequiresDistanceFieldData / RequiresDepthBuffer / RequiresEarlyViewData / RequiresViewUniformBuffer`(L109-112)—— DI 反向查询"需要什么渲染资源",渲染管线据此配 View。返回值决定 `FSceneView` 的构建
- `FeatureLevel ERHIFeatureLevel::Type`(L516)+ `GetFeatureLevel()`(L526)—— 每 Instance 记 GPU feature level,shader 分叉依据(Phase 8)

### 2.5 DI 的 per-instance 数据登记在 SystemInstance 上

Phase 7 会详细讲 DI 的 per-instance data 生命周期。但 Phase 3 必须**登记这个 blob 的存在**——它就住在 SystemInstance 里:

```cpp
// NiagaraSystemInstance.h:426-436
/** 一块大 byte array,所有 DI 的 per-instance 数据共享 */
TArray<uint8, TAlignedHeapAllocator<16>> DataInterfaceInstanceData;

/** 哪些 DI 需要 PreTick / PostTick */
TArray<int32> PreTickDataInterfaces;
TArray<int32> PostTickDataInterfaces;

/** DI* → blob 里的 byte offset 的 Map */
TArray<TPair<TWeakObjectPtr<UNiagaraDataInterface>, int32>> DataInterfaceInstanceDataOffsets;

/** 每脚本类型(Spawn/Update/Event)一份的 "per-instance DI 函数绑定表" */
TArray<FNiagaraPerInstanceDIFuncInfo> PerInstanceDIFunctions[(int32)ENiagaraSystemSimulationScript::Num];

// L212-219 访问
void* FindDataInterfaceInstanceData(UNiagaraDataInterface* Interface);
// L221
const FNiagaraPerInstanceDIFuncInfo& GetPerInstanceDIFunction(ENiagaraSystemSimulationScript ScriptType, int32 FuncIndex) const;
```

**要点**:
- DI 的 per-instance data **不是** DI 自己管的,而是 SystemInstance 分配一大块 byte buffer,按 offset 分给各 DI
- 16 字节对齐是为了 SIMD / GPU 上传
- `PreTick / PostTick` 数组是"本次 tick 要先调 Pre、再调 Post 的 DI 列表"——不是所有 DI 都要这两个钩子
- `PerInstanceDIFunctions[ScriptType][FuncIndex]` 是 System 级脚本里 "per-instance 函数调用" 的绑定表(Phase 5 §3 会展开 `PerInstanceFunctionHook`)
...
- **GT 阶段**(必须在 GameThread):
  ...
  - 读 `DeltaSeconds` / 世界时间
  - `TickInstanceParameters_GameThread` 聚合 per-instance 参数到 `GatheredInstanceParameters` 结构(见下方)
  - 不做重型计算

`GatheredInstanceParameters` 是 SystemInstance 的一个内嵌 public struct(源 L537-564),就是把这帧需要 concurrent 阶段消费的 per-instance 信息打成一包:

```cpp
struct FInstanceParameters
{
    FTransform ComponentTrans = FTransform::Identity;  // Component 当前 Transform
    float DeltaSeconds = 0.0f;
    float TimeSeconds = 0.0f;                          // World.GetTimeSeconds()
    float RealTimeSeconds = 0.0f;                      // World.GetRealTimeSeconds()
    int32 EmitterCount = 0;
    int32 NumAlive = 0;                                // 还活着的 emitter 数
...
Concurrent 阶段的代码只读这个打包好的快照,**不再回头读 GT 的任何东西**——这是"绝不从 worker 线程跨线程读 Component 状态"的硬约束。
- **Concurrent 阶段**(任意线程):
...
**Simulation 层也有一组对应的 Wait**(作用域更粗):

```cpp
// NiagaraSystemSimulation.h:270-272
void WaitForSystemTickComplete(bool bEnsureComplete = false);    // 等 System 级脚本跑完
void WaitForInstancesTickComplete(bool bEnsureComplete = false); // 等所有 Instance 的 Tick_Concurrent 跑完
```

两层的分工:Instance 级 Wait 用于"我要安全访问这个特效",Simulation 级 Wait 用于"我要安全销毁整个 Simulation / 切 Asset"。bug 猎场景下经常需要在两层都加 Wait。
...
> Asset 层 `UNiagaraEmitter::SimTarget` 在运行时体现为:要么 `SpawnExecContext + UpdateExecContext` 有效(CPU),要么 `GPUExecContext` 有效。两条路径不共存。这回应了 [[Wiki/Concepts/UE/Niagara/Niagara-cpu-vs-gpu模拟]] 里的"分叉点"预告。
...
Emitter 还有一组 **DirectBinding 快捷通道**(L174-179),直接把脚本里的特定参数绑到 C++ 字段,**绕开 ParameterStore 查表**:
...
FNiagaraParameterDirectBinding<int32> SpawnExecCountBinding;     // 本帧 Spawn 脚本要跑多少次
FNiagaraParameterDirectBinding<int32> UpdateExecCountBinding;    // 本帧 Update 脚本要跑多少次
```

这是 Niagara"无字符串查表"的另一形态——`FNiagaraParameterStoreToDataSetBinding` 是 **N 个变量打包** 的 memcpy,`FNiagaraParameterDirectBinding<T>` 是 **单变量直通** 的指针绑定。两者用途互补。
...
```cpp
// NiagaraEmitterInstance.h:28-42(private 嵌套结构)
struct FEventInstanceData
{
    ...
    TArray<bool> UpdateEventGeneratorIsSharedByIndex;   // 生成器是否跨 handler 共享(优化 spawn 合并)
...
伴随的还有 `TSharedPtr<const FNiagaraEmitterCompiledData> CachedEmitterCompiledData`(L181)——Asset 侧 `FNiagaraEmitterCompiledData` 的**运行时 shared 引用**。这是 Emitter 从 Asset 拿"编译产物"的桥:脚本字节码、参数布局、DataSet schema 全在里面。读 `FNiagaraEmitterCompiledData` 要走这个指针,不是直接去 `CachedEmitter->GetCompiledData()`——因为 Pool 场景下 Asset 可能已经换人了,Instance 需要自己的**稳定快照**。

### 4.8 Emitter 也有自己的 ExecutionState(简化版双态机)

§2.2 讲的是 SystemInstance 的**双**状态机(Requested/Actual)。Emitter 层**也有**自己的状态机,但是**单态、非双态**:

```cpp
// NiagaraEmitterInstance.h:232
ENiagaraExecutionState ExecutionState = ENiagaraExecutionState::Inactive;

// NiagaraEmitterInstance.h:83-85 helpers
...
// NiagaraEmitterInstance.h:120-121
ENiagaraExecutionState GetExecutionState() { return ExecutionState; }
void SetExecutionState(ENiagaraExecutionState InState);

// NiagaraEmitterInstance.h:70 tick 门禁
FORCEINLINE bool ShouldTick()const { return ExecutionState == ENiagaraExecutionState::Active || GetNumParticles() > 0; }
```

**为什么 Emitter 不需要 Requested/Actual 双态?** 因为:
- 用户 API(`Deactivate`、`Complete`)是**在 SystemInstance 层**暴露的,Emitter 没有直接对外接口
- Emitter 的状态变更要么被 System 级脚本 drive(System Update 里 `SetEmitterEnable`),要么由 Scalability 系统设置
- 因此 "用户意图 vs 实际" 的解耦没必要,单态足够

还有一个**潜伏的 reset pending**位(L235):
```cpp
uint32 bResetPending : 1;
```
注释说"Typical resets must be deferred until the tick as the RT could still be using the current buffer"——**和 SystemInstance 的 `DeferredResetMode` 性质一致**,避免异步期间直接踹翻渲染线程读的 buffer。

### 4.9 运行时统计与 bitfield

Emitter 身上挂一堆运行时计数(L213-237),方便性能分析和调试:

```cpp
float EmitterAge;
int32 InstanceSeed;                  // 随机种子(每 Instance 唯一)
int32 TickCount;
int32 TotalSpawnedParticles;
uint32 CPUTimeCycles;                // Tick 消耗的 CPU 周期
uint32 MaxRuntimeAllocation;
int32 MaxAllocationCount;
int32 MinOverallocation;             // DataSet 分配超额检测
int32 ReallocationCount;
uint32 MaxInstanceCount;

uint32 bResetPending : 1;            // 请求在下次 tick 时 reset(RT 安全)
uint32 bCombineEventSpawn : 1;       // 合并多次 event spawn 为一次(ExecIndex 无关时安全)
```

`GetTotalCPUTimeMS()` / `GetTotalBytesUsed()` 暴露给 stat 系统。Dump 路径靠 `Dump()`(L136)和 `WaitForDebugInfo()`(L138)。

> [!abstract] §4 小结
> Emitter 的核心不是状态机(单态足够),而是**每帧 4 步(PreTick/Tick/PostTick/HandleCompletion)+ 核心字段三元组(SpawnCtx/UpdateCtx/GPUCtx 二选一)+ 粒子 DataSet + Event 数据交换 + Renderer 桥**。状态机交给 System 层统筹,Emitter 层只需要响应。
...
**实例成员实际不是一个 `TArray`,而是 4 个**(源 L393-400)+ 1 个升级队列:

```cpp
TArray<FNiagaraSystemInstance*> SystemInstances;              // 正在跑
TArray<FNiagaraSystemInstance*> SpawningInstances;            // 本帧正在 spawn 窗口(out-of-tick spawn)
TArray<FNiagaraSystemInstance*> PausedSystemInstances;        // SetPaused(true) 的
TArray<FNiagaraSystemInstance*> PendingSystemInstances;       // 排队等首次 spawn(刚 Activate 还没进 Simulation)
TArray<FNiagaraSystemInstance*> PendingTickGroupPromotions;   // TickGroup 升级队列(§5.5)
```

对外用:`AddInstance / RemoveInstance / PauseInstance / UnpauseInstance / TransferInstance / AddPendingSystemInstance / AddTickGroupPromotion`。Instance 在这几个数组之间迁移,状态流转不止 "Active↔Inactive" 二元。

### 5.2 Solo 与 Batched 的双标志(`bSolo` / `bForceSolo`)

回到 §2.2 的"双状态机"范式——Instance 侧还有一对**和 Requested/Actual 完全平行的设计**:

```cpp
// NiagaraSystemInstance.h:457-458
uint32 bSolo : 1;        // 当前实际是否 solo(Simulation 决定)
uint32 bForceSolo : 1;   // 用户强制要求 solo(Phase 2 警告的那个)
```

`SetSolo(bInSolo)`(L74)可以在运行时切换,此时 `TransferInstance` 把 Instance 从旧 Simulation(同 Asset 共享)迁到新 Simulation(per-Instance 独占)。注意 Simulation 自己也有 `bIsSolo`(`NiagaraSystemSimulation.h` L410),这是"这个 Simulation 是 solo 式"——一个 Asset 可以**同时**存在一个 batched Simulation 和 N 个 solo Simulation,取决于每个 Instance 的 `bSolo`。

| | `bForceSolo` | `bSolo` |
|---|---|---|
| 谁设 | 用户(Component `bForceSolo` 或 `AdvanceSimulation` 要求) | 系统根据 `bForceSolo` + legacy ExecContext CVar + 是否能进共享 Simulation 决定 |
| 何时读 | `Init()` 初始化阶段 | `IsSolo()` 查询、每次决策批量与否 |

这就解释了 §5.6 的性能退化:`bForceSolo = true` → `bSolo = true` → 该 Instance 拿一个独立 Simulation → 批处理效益归零。

### 5.3 Tick Batch = 4 的秘密
...
### 5.4 Tick Group Promotion:Instance 可跨帧升 TickGroup

Simulation 的身份含 `SystemTickGroup`——但 Instance 的"合适 TickGroup"可能因 **DI prereq / AttachParent 变化** 动态改变。运行时机制:

```cpp
// NiagaraSystemInstance.h
void UpdatePrereqs();                                   // L78
ENiagaraTickBehavior GetTickBehavior() const;           // L315(默认 UsePrereqs)
void SetTickBehavior(ENiagaraTickBehavior);             // L317(延后到下次评估再生效)
ETickingGroup CalculateTickGroup() const;               // L320,算出"这个 Instance 这帧该在哪个 TickGroup"

// NiagaraSystemSimulation.h
void UpdateTickGroups_GameThread();                     // L261
void AddTickGroupPromotion(FNiagaraSystemInstance*);    // L299
TArray<FNiagaraSystemInstance*> PendingTickGroupPromotions;  // L403
```

工作流:

1. 某 Instance 发现自己该升到更后的 TickGroup(例如它 attach 的 Actor 在 `TG_PostPhysics` 才更新,Niagara 要在那之后跑)
2. 调 `AddTickGroupPromotion` 挂到 Simulation 的升级队列
3. 下一帧 `UpdateTickGroups_GameThread` 处理队列,把 Instance 从当前 Simulation 迁到目标 TickGroup 对应的 Simulation(可能是**同 Asset 不同 TickGroup 的另一个 Simulation**)

这也是为什么 WorldManager 的索引是 `(Asset, World, **TickGroup**)` 三元组——同一 Asset 在不同 TickGroup 下是**不同的 Simulation**。Instance 可以跨帧在这些 Simulation 间迁移。

> [!tip] 与 Phase 2 `TickBehavior` 的接线
> `ENiagaraTickBehavior { UseComponentTickGroup / ForceTickFirst / ForceTickLast / UsePrereqs }` 在 Component 层(Phase 2)设置。`UsePrereqs` 时,`CalculateTickGroup` 会读 DI 的 `CalculateTickGroup()`(Phase 7)+ `PrereqComponent`(L384)的 TickGroup,综合决定。这是 Niagara 和 UE Tick 系统握手的关键点。

### 5.5 System 级脚本的 Pipeline

Simulation 其实有**两条并列 pipeline**——一条常规 Tick,一条 Spawn:

```cpp
// —— Tick pipeline(每帧,处理 SystemInstances)——
...
// —— Spawn pipeline(处理 PendingSystemInstances/SpawningInstances,out-of-tick spawn 用)——
Spawn_GameThread(DeltaSeconds, bPostActorTick)
    → Spawn_Concurrent(Context)        // 同样支持并发
```

两条 pipeline 都吃一个 **`FNiagaraSystemSimulationTickContext`** 结构(源 L207-236)打包所有共享状态:

```cpp
struct FNiagaraSystemSimulationTickContext {
    FNiagaraSystemSimulation* Owner;
    UNiagaraSystem* System;
    TArray<FNiagaraSystemInstance*>& Instances;   // 本次要处理的实例列表
    FNiagaraDataSet& DataSet;                     // 本次用哪份 DataSet
    float DeltaSeconds;
    int32 SpawnNum;
    int  EffectsQuality;
    FGraphEventRef MyCompletionGraphEvent;
    FGraphEventArray* FinalizeEvents;
    bool bTickAsync;                              // Tick_Concurrent 是否真的走 worker
    bool bTickInstancesAsync;                     // 嵌套的 Instance Tick_Concurrent 是否并发
};
```

两个工厂:`MakeContextForTicking` / `MakeContextForSpawning` 分别造 Tick / Spawn pipeline 的 Context。

### 5.6 参数 ↔ DataSet 绑定的运行时形态
...
Simulation 里持有 **2 + 5 共 7 组 binding**,方向正好相反:

```cpp
// NiagaraSystemSimulation.h:366-379
// —— Component 参数 → System DataSet(输入,PrepareForSystemSimulate 用)——
...
// —— System DataSet → 各 Emitter 参数存储(输出,TransferSystemSimResults 用)——
...
TArray<TArray<FNiagaraParameterStoreToDataSetBinding>> DataSetToEmitterEventParameters;  // 双层 TArray:per-emitter × per-event-handler
...
- **前 2 组**是 "每个 Component 实例的参数 → System 层 DataSet" 的聚合;`PrepareForSystemSimulate` 调用。
- **后 5 组**是 "System 层计算结果 → 每个 Emitter 的 4 套参数存储(Spawn/Update/Event/GPU)+ Renderer 绑定" 的派发;`TransferSystemSimResults` 调用。
- Event 一项**双层 `TArray`**:外层 per-emitter,内层 per-event-handler。

"System attribute 对 Emitter 可见"的运行时机制就是这后 5 组 binding 把 System DataSet 的结果 memcpy 到各 Emitter 的参数存储。

除此之外还有一组并列的 `FNiagaraConstantBufferToDataSetBinding`(源 L194-205),专门从常量 buffer 拷到 DataSet,跟 `FNiagaraParameterStoreToDataSetBinding` 形成"直接参数 store vs 结构化常量 buffer"两条供数路径。

**Simulation 还持有 6 个 Engine 变量的 `FNiagaraParameterDirectBinding<T>`**(源 L383-390)——这是 System 级脚本里引用的"引擎级"变量直通:

```cpp
FNiagaraParameterDirectBinding<int32> SpawnNumSystemInstancesParam;       // "有多少 instance"
FNiagaraParameterDirectBinding<int32> UpdateNumSystemInstancesParam;

FNiagaraParameterDirectBinding<float> SpawnGlobalSpawnCountScaleParam;    // Scalability 全局 spawn scale
FNiagaraParameterDirectBinding<float> UpdateGlobalSpawnCountScaleParam;

FNiagaraParameterDirectBinding<float> SpawnGlobalSystemCountScaleParam;   // Scalability 全局 instance count scale
FNiagaraParameterDirectBinding<float> UpdateGlobalSystemCountScaleParam;
```

Spawn / Update 各一份——因为这两个脚本是独立 compile 的,各自维护各自的 parameter offset。`SetupParameters_GameThread` 里这 6 个 binding 的 `SetValue` 被调用,值直接写进 ParameterStore 的底层 byte buffer,下一秒 `UpdateSystemInstances` 里脚本读到就是最新值。Scalability 怎么生效的?Phase 9 `FNiagaraScalabilityManager` 就是在 Tick 前改这两个全局 scale 的源。

### 5.7 多份 DataSet

```cpp
FNiagaraDataSet MainDataSet;                     // 常规 tick
FNiagaraDataSet SpawningDataSet;                 // out-of-tick spawn(任何"非正常 tick 窗口里要 spawn 新实例"的情形都走这里)
...
2. **两对双状态(Requested/Actual 状态机 + bForceSolo/bSolo 标志)** 都是"用户意图 vs 系统实际"的解耦——典型 UE 模式。Scalability cull 走 Actual 不动 Requested,避免误发 `OnSystemFinished`;SetSolo 通过 TransferInstance 实现状态切换
3. **Emitter 的状态机是单态**——因为它没有直接对外 API,只响应 System 层决策,所以不需要双态
4. **三阶段 Tick 的分工**:GT 收输入、Concurrent 做重活、Finalize GT 收尾 + GPU 提交——避免 GT 阻塞但保持 DI 后处理的顺序性。Concurrent 阶段只读 `GatheredInstanceParameters` 快照,绝不回读 GT 状态
5. **参数双缓冲**解决渲染线程读上一帧、游戏线程写当前帧的竞态;`CurrentFrameIndex : 1` 位 flip,无锁
6. **Simulation 的身份 = (Asset, World, TickGroup)**,同 Asset 多实例自动聚到一个 Simulation,批量 tick 4 个一组。实例成员不是一个 `TArray`,而是 4 个 + 1 升级队列(Active/Pending/Paused/Spawning + PendingTickGroupPromotions)
7. **Tick Group Promotion** 支持 Instance 跨帧升 TickGroup——依赖 DI 或 AttachParent 的 TickBehavior 变了,Instance 通过 `AddTickGroupPromotion` 挂队,`UpdateTickGroups_GameThread` 把它迁到目标 TickGroup 的另一 Simulation
8. **`bForceSolo` 退化 5-20 倍**——Solo 模式等于把 50 个 instance 打回 50 个 Simulation,System 级脚本、绑定、Engine DirectBinding 都不能共享
9. **2 + 5 共 7 组 `FNiagaraParameterStoreToDataSetBinding` + 6 个 Engine `FNiagaraParameterDirectBinding<T>`** 是 Niagara "无字符串查表"在 Phase 3 的具体形态——每帧 N 实例 × M 参数的移动全靠预算好的 offset memcpy,Scalability 全局 scale 直接写底层 byte buffer
10. **DI 的 per-instance data 不归 DI 管**——SystemInstance 持有一块 16 字节对齐的 byte blob,按 offset 分给各 DI;`PreTick / PostTick` 两个数组只含需要对应钩子的 DI 子集,避免空转
...
- 源摘要:
  - [[Wiki/Sources/Stock/Niagara/NiagaraSystemInstance]](573 行,中间层)
  - [[Wiki/Sources/Stock/Niagara/NiagaraEmitterInstance]](238 行,最内层)
  - [[Wiki/Sources/Stock/Niagara/NiagaraSystemSimulation]](428 行,最外层批量调度)
- Entity 页:
  - [[Wiki/Entities/Stock/Niagara/FNiagaraSystemInstance]]
  - [[Wiki/Entities/Stock/Niagara/FNiagaraEmitterInstance]]
  - [[Wiki/Entities/Stock/Niagara/FNiagaraSystemSimulation]]

### 前置议题

- Phase 2 Component 层:[[Readers/UE/Niagara/Phase 2 - Component 层的五职责]] — `UNiagaraComponent::SystemInstance` 的兑付
- Phase 1 Asset 层:[[Readers/UE/Niagara/Phase 1 - 从 System 到图源抽象基类]] — `FNiagaraSystemCompiledData` 的绑定在 §5.4 被消费
- Phase 0 心智模型:[[Readers/UE/Niagara/Phase 0 - 上阵前的四层脑内地图]] — CPU/GPU 分叉、Asset/Instance 二元
...
*本读本由 [[Claudian]] 基于 Phase 3 的 3 个头文件(573+238+428 行)综合生成,2026-04-20。commit `b6ab0dee9`。*
