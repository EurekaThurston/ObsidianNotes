...
> 本页是 Niagara 学习路径 [[Wiki/Syntheses/UE/Niagara/Niagara-learning-path]] Phase 1 的**主题读本**——详细、精确、满满当当,一次读完即完整掌握 Niagara Asset 层(System / Emitter / Script)的全部心智模型,不需要跳转。
>
...
Phase 0 建立了几个抽象概念:[[Wiki/Concepts/UE/UE4-资产与实例|Asset vs Instance]]、[[Wiki/Concepts/UE/Niagara/Niagara-cpu-vs-gpu模拟|CPU 还是 GPU 模拟]]、[[Wiki/Concepts/UE/Niagara/Niagara-vs-cascade|Niagara 为什么换掉 Cascade]]——都是**脑内地图**,没读一行代码。

Phase 1 回答一个非常具体的问题:

> [!question] Phase 1 要回答
...
>
> ——即"此类是运行时 `FNiagaraEmitterInstance` 需要序列化的那部分属性"。这是纯粹的 "Asset describes Instance" 模型:Asset 是模板,Instance 是基于模板在 World 里实例化出来的活物。
...
这就是 [[Wiki/Concepts/UE/Niagara/Niagara-cpu-vs-gpu模拟]] 里讲过的那个分叉点——**同一个 Niagara Emitter 资产,仅仅改这一个字段,走的就是两套完全不同的执行路径**:
...
它的 `operator==` + `GetTypeHash` 被用作 [[Wiki/Concepts/UE/UE4-ddc|DDC(Derived Data Cache)]] 的 key——UE 的 DDC 是**机器级的编译产物缓存**,可以在团队间共享。两个人编译同一个图,只要这个 Id 一样,第二个人从 DDC 拉缓存即可,不用真跑编译。
...
### 原子源摘要(逐文件)
- [[Wiki/Sources/Stock/Niagara/NiagaraSystem]] — `NiagaraSystem.h` 全字段清单 + 代码片段引用
- [[Wiki/Sources/Stock/Niagara/NiagaraEmitter]] — `NiagaraEmitter.h` 同上
- [[Wiki/Sources/Stock/Niagara/NiagaraEmitterHandle]] — `NiagaraEmitterHandle.h` 同上
- [[Wiki/Sources/Stock/Niagara/NiagaraScript]] — `NiagaraScript.h` 同上(含编译产物详解)
- [[Wiki/Sources/Stock/Niagara/NiagaraScriptSourceBase]] — `NiagaraScriptSourceBase.h` 同上

### 实体页(按"类"索引,跨 source 稳定入口)
- [[Wiki/Entities/Stock/Niagara/UNiagaraSystem]]
- [[Wiki/Entities/Stock/Niagara/UNiagaraEmitter]]
- [[Wiki/Entities/Stock/Niagara/FNiagaraEmitterHandle]]
- [[Wiki/Entities/Stock/Niagara/UNiagaraScript]]
- [[Wiki/Entities/Stock/Niagara/UNiagaraScriptSourceBase]]

### 上下文(Phase 0 概念,前置)
- [[Wiki/Concepts/UE/UE4-uobject-系统]] — UCLASS/USTRUCT/UPROPERTY 宏
- [[Wiki/Concepts/UE/UE4-资产与实例]] — Asset vs Instance 二元模型
- [[Wiki/Concepts/UE/Niagara/Niagara-vs-cascade]] — 设计哲学对比
- [[Wiki/Concepts/UE/Niagara/Niagara-cpu-vs-gpu模拟]] — SimTarget 分叉的根源
