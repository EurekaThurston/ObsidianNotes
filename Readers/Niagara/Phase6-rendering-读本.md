...
> 本页是 Niagara 学习路径 Phase 6 的**主题读本**。一次读完掌握 Niagara Renderer 家族:Asset/Instance 对偶、GT→RT 数据流、4 种具体类型(Sprite/Ribbon/Mesh/Light)的能力边界与陷阱。
...
```cpp
// Runtime mutable 字段,每帧统计
mutable float TessellationAngle = 0;          // 平均段角度
...
系统自动根据曲率+twist 决定**下一帧**的 tessellation factor。配置里:
...
float TessellationAngle;                           // 触发加细的角度阈值
...
- `DistributionMode`(ScaledUniformly / ScaledUsingSegmentLength / TiledOverRibbonLength)
...
- Source × 10:2 基类 + 4 × (Properties + Renderer)
- Entity × 6:2 基类 + 4 具体类型(合并 Properties/Renderer)

### 前置

- Phase 2 Component / Phase 3 Emitter Instance `RendererBindings` / Phase 4 DataSet 和 DataSetAccessor
- Phase 5 Component Render Pool 概念(本 Phase 未深入)
