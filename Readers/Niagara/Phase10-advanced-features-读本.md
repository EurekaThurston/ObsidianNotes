...
> 本页是 Niagara 学习路径**最后一个 Phase** 的主题读本。一次读完掌握 Simulation Stages 多 pass 架构 + Grid 2D/3D 场存储 + NeighborGrid 空间哈希 + Emitter 间数据交互。
...
- Source × 6:SimulationStageBase / DataInterfaceRW / Grid2DCollection / Grid2DCollectionReader / Grid3DCollection / NeighborGrid3D
- Entity × 5:UNiagaraSimulationStage / UNiagaraDataInterfaceRWBase(合并 Grid2D/3D 基类)/ UNiagaraDataInterfaceGrid2DCollection(合 Reader)/ UNiagaraDataInterfaceGrid3DCollection / UNiagaraDataInterfaceNeighborGrid3D

### 前置

- Phase 8.5 `FSimulationStageMetaData`(SimStage 的 shader 侧元数据)
- Phase 7 `UNiagaraDataInterfaceRenderTarget2D`(首个 RW DI 示范)
- Phase 4 DataSet 双缓冲思路
