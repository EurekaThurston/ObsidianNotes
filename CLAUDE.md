...
9. **自验(强制)**:对本轮 log 条目里所有声称"新建 `[[...]]`"的路径,各跑一次 Glob 验证真实存在。**有任何差异立即停下告诉用户**(要么文件没真正落盘、要么记错路径、要么用户手动删除需要修 log)。避免"log 声称建了但 fs 里没有"的幽灵文件。
...
**硬性要求**(见模板 header):线性叙事 / 不遗漏 > 简短 / 关键代码 inline / 问题驱动 / ⚠️ 陷阱埋雷 / 末尾深入阅读 + 下一步 + open questions + **"自检问题(读完回答)"章节** + 签名行。

**自检问题章节**(必有,放在"关键洞察"之后、"留下的问题"之前):
- **目的**:验证读者**真懂**而非"扫读过"——人类读完读本后用这些问题自测
- **题型禁忌**:绝不出"回原文检索某字段/方法签名就能答"的浅题(那种问题查 wiki 即可,不需要读本)
- **题型必须**:综合 / 反推 / 取舍 / 假设题——典型句式:
  - "为什么 X 必须这样设计?反过来设计会撞到什么具体问题?"
  - "在 <场景> 下选 A 还是 B?判断标准?"
  - "把 §a / §b / §c 串起来:<连锁后果> 是怎么发生的?"
  - "<某优化> 的代价转移到了哪里?"
  - "用 5 句话向不熟悉本议题的人解释 <核心概念>"
- **数量**:5-8 题为典型,视议题深度浮动,不强求统一
- **不在文中给答案**——这是给读者自测的
