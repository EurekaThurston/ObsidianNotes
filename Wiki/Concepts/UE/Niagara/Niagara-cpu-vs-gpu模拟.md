...
## 相关
- [[Wiki/Concepts/UE/Niagara/Niagara-vs-cascade]] — Cascade 基本不支持真正的 GPU 模拟；这是 Niagara 的核心优势之一
- [[Wiki/Concepts/UE/UE4-资产与实例]] — CPU/GPU 的选择体现在 Asset（Emitter 的配置）上，运行时实例才真正执行
- [[Wiki/Syntheses/UE/Niagara/Niagara-learning-path]] — Phase 5（CPU VM）和 Phase 8（GPU 模拟）分别深入两条路径
