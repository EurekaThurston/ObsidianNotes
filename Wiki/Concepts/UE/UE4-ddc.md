...
- [[Wiki/Entities/Stock/Niagara/UNiagaraScript|UNiagaraScript]] 的 `CachedScriptVMId`(`FNiagaraVMExecutableDataId`)**直接作为 DDC key**
...
- 详见 [[Wiki/Sources/Stock/Niagara/NiagaraScript]] 的 DDC 一节 + [[Readers/UE/Niagara/Phase 1 - 从 System 到图源抽象基类]] § 5
...
- [[Wiki/Concepts/UE/UE4-uobject-系统|UObject 系统]] — UAsset 序列化层,DDC 在其上
- [[Wiki/Concepts/UE/UE4-资产与实例|资产 vs 实例]] — DDC 只缓存"资产级"的派生数据,不缓存运行时实例状态
- [[Wiki/Entities/Stock/Niagara/UNiagaraScript]] — Niagara 里 DDC 的主要消费者
- [[Wiki/Concepts/UE/Niagara/Niagara-vs-cascade|Niagara vs Cascade]] — "显式编译字节码 + DDC 缓存"是 Niagara 相对 Cascade 的质变之一

## 深入阅读

- 源引用:[[Wiki/Sources/Stock/Niagara/NiagaraScript]] — Niagara DDC key 构造的代码级细节
- 主题读本:[[Readers/UE/Niagara/Phase 1 - 从 System 到图源抽象基类]] § 5(UNiagaraScript)
...
- 本页从 [[Wiki/Sources/Stock/Niagara/NiagaraScript]] 对 DDC 的使用方式反推而来。DDC 本身的源码(`Engine/Source/Runtime/DerivedDataCache/`)尚未作为独立源 ingest。如后续需要,可补 ingest 为 [[Wiki/Sources/Stock/Niagara/]] 下的代码源。
