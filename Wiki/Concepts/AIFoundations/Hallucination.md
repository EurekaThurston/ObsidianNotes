...
[[Wiki/Concepts/AIFoundations/Llm|LLM]] 的核心动作是"猜下一个词"，它**没有"知道"与"不知道"的概念**。当训练数据里没有准确答案时，它也会顺着语气编一个——而且编得**又流畅又自信**。这就是幻觉。
...
- **工具调用 / [[Wiki/Concepts/AIFoundations/Mcp|MCP]]**：让 AI 去真实系统查数据（查数据库、跑脚本、搜网页），而不是靠记忆。
- **[[Wiki/Concepts/AIFoundations/Harness-engineering|Harness Engineering]] 的反馈回路**：让 AI 的产出被测试/Linter/类型检查等自动验证，错了就强制改。
- **HITL（Human-in-the-Loop）**：关键决策点叫人审核。
- **[[Wiki/Concepts/AIFoundations/Reasoning-model|推理模型]]**：通过显式"打草稿"减少低级幻觉；但对"编造事实"这类幻觉效果有限。
...
- [[Wiki/Concepts/AIFoundations/Llm|LLM]] — 幻觉的载体
- [[Wiki/Concepts/AIFoundations/Context-window|上下文窗口]] — 上下文塞得过满也会诱发类似"幻觉"的失忆
- [[Wiki/Concepts/AIFoundations/Harness-engineering|Harness Engineering]] — 用工程机制对抗幻觉
