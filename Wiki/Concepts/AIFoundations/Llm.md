...
**重要澄清**：LLM 没有"真的在想"，也没有"知道 vs 不知道"的自觉。这是理解它为什么会 [[Wiki/Concepts/AIFoundations/Hallucination|产生幻觉]]、为什么有[[Wiki/Concepts/AIFoundations/Context-window|上下文腐烂]]、为什么同一问题两次答不一样的钥匙。

## 关键事实 / 属性

- **底层架构**：Transformer（2017，Google《Attention is All You Need》）。自注意力机制让模型可以"一眼扫过整段话"，而不是从左到右逐字。
- **处理单元**：[[Wiki/Concepts/AIFoundations/Context-window|token]]——粗略地说英文 1 token ≈ 0.75 个单词，中文 1 汉字 ≈ 1-2 token。
- **上限**：[[Wiki/Concepts/AIFoundations/Context-window|上下文窗口]]——一次能看多少 token。当前旗舰已到数十万至百万级。
...
- [[Wiki/Concepts/AIFoundations/Hallucination|幻觉]] — LLM 的结构性毛病
- [[Wiki/Concepts/AIFoundations/Context-window|上下文窗口 & Context Rot]] — LLM 的"视力范围"及其陷阱
- [[Wiki/Concepts/AIFoundations/Reasoning-model|推理模型]] — 2024 末起的新变种，先"打草稿"再答
- [[Wiki/Concepts/AIFoundations/Ai-agent|AI Agent]] — 把 LLM 装上手脚
