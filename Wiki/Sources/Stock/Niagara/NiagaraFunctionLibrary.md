...
深入留给 [[Wiki/Sources/Stock/Niagara/NiagaraScriptExecutionContext]](Phase 5)时展开。
...
- [[Wiki/Entities/Stock/Niagara/UNiagaraFunctionLibrary]] — 本文件定义的主类
- [[Wiki/Entities/Stock/Niagara/UNiagaraComponent]] — Spawn 的返回类型与参数载体
- [[Wiki/Entities/Stock/Niagara/UNiagaraSystem]] — Spawn 的 Template 参数
- [[Wiki/Concepts/UE/UE4-资产与实例]] — Spawn = Asset → Component → Instance 的标准链路

## 与既有 wiki 的关系

- 是 [[Wiki/Sources/Stock/Niagara/NiagaraComponent]] 的 BP 外立面——FunctionLibrary 本身不持有状态,所有状态都在返回的 Component 对象上
- VectorVM Fast Path 的存在印证了 [[Wiki/Concepts/UE/Niagara/Niagara-cpu-vs-gpu模拟]] 中 "CPU 模拟不等于慢"——针对常用算子有专门快速路径
...
- `bPreCullCheck=true` 默认开会不会造成"生成即消失"的错觉?scalability 的 PreCull 决策表 → [[Wiki/Sources/Stock/Niagara/NiagaraScalabilityManager]](Phase 9)
