...
- `UNiagaraComponent : public UFXSystemComponent`(L53):Niagara 在场景里的承载者
  - `UFXSystemComponent` 是 UE 通用的特效组件基类,Cascade 的 `UParticleSystemComponent` 同样继承它——又一个与 [[Wiki/Entities/Stock/Niagara/UNiagaraSystem]] `UFXSystemAsset` 对应的"历史兼容面"
...
- [[Wiki/Entities/Stock/Niagara/UNiagaraComponent]] — 本文件定义的主类
- [[Wiki/Entities/Stock/Niagara/UNiagaraSystem]] — Component 引用的 Asset
- [[Wiki/Entities/Stock/Niagara/ANiagaraActor]] — Component 的 Actor 封装
- [[Wiki/Entities/Stock/Niagara/UNiagaraFunctionLibrary]] — BP 入口
- [[Wiki/Concepts/UE/UE4-资产与实例]] — Component 是 Asset→Instance 转换的承载者
- [[Wiki/Concepts/UE/UE4-uobject-系统]] — UCLASS / UPROPERTY / UFUNCTION 大量使用

## 与既有 wiki 的关系

- 延伸了 [[Wiki/Syntheses/UE/Niagara/Niagara-learning-path]] 的三层架构图:Asset(Phase 1)与 Instance(Phase 3)之间的**承载层**,此前一笔带过
- 印证了 [[Wiki/Concepts/UE/UE4-资产与实例]] 中 "同一 Asset 可对应多个运行时对象":一个 `UNiagaraSystem` 可被多个 `UNiagaraComponent` 同时引用,每个 Component 独占一个 `FNiagaraSystemInstance`
...
- `bForceSolo` 究竟绕开了多少优化?批量 tick 共享哪些东西?→ 看 [[Wiki/Sources/Stock/Niagara/NiagaraSystemSimulation]](Phase 3)
- `FNiagaraScalabilityManager` 被 friend 访问的是哪些字段?设计权衡?→ 看 [[Wiki/Sources/Stock/Niagara/NiagaraScalabilityManager]](Phase 9)
- `ENCPoolMethod` 五种取值的决策表是什么?何时哪种?→ 看 [[Wiki/Sources/Stock/Niagara/NiagaraComponentPool]](Phase 9)
